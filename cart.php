...
switch ($op){
  ...
  case "order_insert" :
    ...
    redirect_header($returnUrl, "下單成功", 2000, 'success');    
    exit;
    
  case "order_update" :    
    if($_SESSION['user']['kind'] !== 1)redirect_header("index.php", '您沒有權限', 2000, 'warning');
    $returnUrl = order_insert($sn);
    redirect_header($returnUrl, "訂餐編輯成功", 2000, 'success');    
    exit; 
  
	case "add_cart" :
    $msg = add_cart($sn);
    redirect_header("cart.php", $msg, 2000, 'success');    
    exit;
...
function order_insert($sn=""){
  ...
  if($_POST['op'] == "order_insert"){
    ...
    $lineId = "VcVZxfDpdspk1bnkuDmSSjT55HSJknRzdHUGQwMn0hO";
